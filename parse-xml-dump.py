...
class dbController:
	def __init__(self):
		self.con = mdb.connect('localhost','root','1m4agrncahu','citeseer')
...
def read_record():
    try:
        with open('../dataset/block_record2.txt') as f:
        	block = f.read()	        	
        	return int(block)	        	   
    except IOError:
        return 0
...
class CiteseerxHandler(xml.sax.ContentHandler):
	...
	def block_record(self):	    
	    target = open('../dataset/block_record2.txt','w')
